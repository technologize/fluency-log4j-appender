...
public class FluencyAppender extends AbstractAppender {
  ...
  protected FluencyAppender(final String name, final String tag, final Field[] fields,
      final FluencyConfig fluencyConfig, final Filter filter,
      final Layout<? extends Serializable> layout, final String ignoreExceptions) {
    ...
    for (Field field : fields) {
      ...
      if (Assert.isNonEmpty(field.getPattern()) && Assert.isNonEmpty(field.getName())) {
        PatternLayout valueLayout =
            PatternLayout.newBuilder().withPattern(field.getPattern()).build();
  ...
  public void append(LogEvent logEvent) {
    ...
    if (Assert.isEmpty(logSource)) {
      ...
      logEventData.put("sourceFile",
          Assert.isNonEmpty(logSource.getFileName()) ? logSource.getFileName() : UNKNOWN);
      logEventData.put("sourceClass",
          Assert.isNonEmpty(logSource.getClassName()) ? logSource.getClassName() : UNKNOWN);
      logEventData.put("sourceMethod",
          Assert.isNonEmpty(logSource.getMethodName()) ? logSource.getMethodName() : UNKNOWN);
      logEventData.put("sourceLine",
          Assert.isNonEmpty(logSource.getLineNumber()) ? logSource.getLineNumber() : 0);
    }
    ...
    /*
     * Refer: https://www.elastic.co/guide/en/elasticsearch/reference/current/date.html Format that
     * supports nanos is strict_date_optional_time_nanos which is same as
     * DateTimeFormatter.ISO_INSTANT
     */
    logEventData.put("@timestamp", DateTimeFormatter.ISO_INSTANT.format(Instant.ofEpochSecond(
        logEvent.getInstant().getEpochSecond(), logEvent.getInstant().getNanoOfSecond())));

    if (!contextData.isEmpty()) {
      Map<String, String> map = contextData.toMap();
      for (String k : map.keySet()) {
        logEventData.put(k, map.get(k));
      }
    }
